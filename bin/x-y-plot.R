...
if(!exists("na")) { # na string
  na="NA"
}

if(!exists("title")) { # title for the plot
  title=""
}
...
dataclasses[!names(data) %in% c(xheader,yheader)]<-"NULL"
...
# Make the plot
qplot(data[,xheader],data[,yheader],ylab=yheader,xlab=xheader,main=title)
